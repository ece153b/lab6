...
void UART2_GPIO_Init(void) {
	//Enable Clock for PA 2 (tx/transmitter) and Pa 3 (RX/reciever)
...
void USART_Init(USART_TypeDef* USARTx) {
	...
	//Set word lenght to 8 bits
	USARTx->CR1 &= ~(USART_CR1_M);
	//Oversampling mode to oversample by  16
	USARTx->CR1 &= ~(USART_CR1_OVER8);
	//Num of stop bits to 1
	USARTx->CR2 &= ~(USART_CR2_STOP);
	//Set Baud Rate to 9600
	USARTx->BRR = 8333;
	//Enable both transmitter and reciever in control register
	USARTx->CR1 |= USART_CR1_TE;
	USARTx->CR1 |= USART_CR1_RE;
	//Reenable USARtx back 
	USARTx->CR1 |= USART_CR1_UE;
}
